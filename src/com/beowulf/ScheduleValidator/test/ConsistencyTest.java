			...
			if (constraintmap.get(idBoth) != null) {
				...
				if (!defsOnly) {
					...
					if ((short) (vektor & allen) == 0) {
						...
						errorMessages.add("[Conflict] The constraint says: " + rel.getX1().getName() + ", "
								+ rel.getX1().getProf() + " " + rel.getX1().getCourse()
								+ myConstraintNetwork.getConstraintStringFromConstraintShort(vektor) + " "
								+ rel.getX2().getName() + ", " + rel.getX2().getProf() + " " + rel.getX2().getCourse());
